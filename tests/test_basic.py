...
class TestBasic(object):
    ...
    def test_lock(self):
        """Test CDR lock, by ensuring that last 20% of locked indication vector is all True."""
        _lockeds = self.dut.lockeds
        assert all(_lockeds[4*len(_lockeds)//5:]), "CDR lock is unstable!"

    def test_adapt(self):
        """Test DFE lock, by ensuring that last 20% of all coefficient vectors are stable to within +/-20% of their mean."""
        ...
        assert all(map(lambda pr: all(abs(pr[0] - pr[1])/pr[1] < 0.2), zip(_ws, _means))), "DFE adaptation is unstable!"
